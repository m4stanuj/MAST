...
_DANGER_PATTERNS = [
    ...
    # M4STCLAW specific
    (r"del.*memory\.json",              "deleting memory file — all M4STCLAW memory lost"),
    ...
    for path_pat in _PROTECTED_PATHS:
        if re.search(path_pat, cmd, re.IGNORECASE):
            if any(op in cmd_lower for op in ["rm ", "del ", "delete", "rmdir", "erase"]):
                return False, "block", f"⛔ BLOCKED — Touching protected M4STCLAW file: {path_pat}"
